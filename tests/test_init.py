...
async def test_setup_unload_and_reload_entry(hass, bypass_get_trackers):
    ...
    assert (
        type(hass.data[DOMAIN][config_entry.entry_id]) == WeenectDataUpdateCoordinator
    )
...
async def test_setup_entry_exception(hass, error_on_get_trackers):
