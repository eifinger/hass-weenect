...
async def test_sensor(hass):
    ...
    assert hass.states.get("sensor.test_sensor_mode").attributes["attribution"] == ATTRIBUTION
    assert hass.states.get("sensor.test_sensor_mode").attributes["id"] == "100000"
    assert hass.states.get("sensor.test_sensor_mode").attributes["sim"] == "8849390213023093728"
    assert hass.states.get("sensor.test_sensor_mode").attributes["imei"] == "160389554842512"
    ...
    assert hass.states.get("sensor.test_last_message_received").state == "2021-04-15T08:29:28+00:00"
    ...
    assert hass.states.get("sensor.test_subscription_expiration_date").state == "2022-09-21T11:34:12+00:00"
    ...
    assert hass.states.get("sensor.test_next_charge").state == "2022-09-21T11:34:12+00:00"
...
async def test_device_class_does_not_return_string_for_its_state(hass, caplog):
    ...
    assert "is providing a string for its state, while the device class is" not in caplog.text
